...
We can derive the implied total number of votes cast from the number of votes received by a candidate and the percentage of votes received by that candidate, *i.e.,* $votes\_cast\ = \frac{general\_votes}{general\_percent}$.  We would expect that number to be the same when calcuilated across every observation for a particular race.
...
results_house %>%
  ...
  filter(unique_votes_cast > 1)
```
We are down to just suspicious contests.  Let's take a closer look.
...
vote_reconciliation <- results_house %>%
  drop_na(general_votes) %>%
...
```
  We have non-conforming district identification numbers in the dataset.  Let's take a look.
  
```{r}
results_house %>% 
  drop_na(general_votes) %>%
  filter (!str_detect(district_id, "^[:digit:]+$")) %>%
  group_by (state, district_id) %>%
  summarise(count = n(), .groups="keep")
```

Since we have been grouping by state and district id numbers, we should fix these labels.
...
Let's repeat out vote total integrity test.
...
df <- inner_join(campaigns, results_house.refined, by="cand_id") %>%
...
vote_plot_3 <- df %>% filter (candidate_count > 1) %>%
  ggplot() +
  aes (x = general_percent) +
  geom_histogram (color = "black", fill = "white", bins = 100, na.rm = TRUE) +
  scale_x_continuous(labels = scales::percent) + 
